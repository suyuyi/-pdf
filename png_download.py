...
import re,requests,time,os,img2pdf,threading,_thread
...
def download_convert_mul(path,ratify_no):
    ...
    img_to_pdf(path)
    tkinter.messagebox.showinfo(title="Info", message="目标文件：" + str(ratify_no)+"已完成下载")
...
image_file = tk.PhotoImage(file=r'C:\Users\song_xinbai\Downloads\19-20春季学期资料\结题报告\test\17.png')
image = canvas.create_image(300, 30, anchor='n', image=image_file)
canvas.pack(side='top')
tk.Label(window, text='请输入绝对路径和编号，然后点击下载', font=('Arial', 16)).pack()
...
path_str.set(r"C:\Users\song_xinbai\Downloads\19-20春季学期资料\结题报告")
